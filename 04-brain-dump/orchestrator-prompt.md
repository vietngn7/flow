...
```json
{
  ...
  "context": {
    ...
    "current_time": "HH:MM",
    "current_day_of_week": "Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday",
    "timezone": "Europe/Kiev",
    "last_mentioned_task_id": "task_id or null",
    "has_existing_tasks": true,
    "user_custom_categories": ["mia", "startup", "podcast"],
    "session_id": "unique_session_id_for_this_brain_dump",
    "recent_operations": [
      {
...
- `context.current_day_of_week`: Day of week (needed by Query Agent for "this week" queries)
- `context.has_existing_tasks`: Whether user has any tasks (impacts routing logic)
- `context.user_custom_categories`: List of custom category names (needed by Query Agent and Brain Dump Parser)
- `context.session_id`: Unique ID for current brain dump session (Brain Dump Parser includes this in task output for deduplication)
...
<context_management>
## System Responsibilities for Context Updates

**IMPORTANT:** The system (application layer) is responsible for maintaining context state between agent calls.

The Orchestrator classifies and routes requests, but does NOT modify the context itself. Context management is handled by the system integration layer.

### Context Update Rules

After each agent execution, the system MUST update context:

**1. After Brain Dump Parser completes:**
```javascript
// Extract last created task ID
const lastTaskId = brainDumpResult.tasks[brainDumpResult.tasks.length - 1].id;

// Update context
context.last_mentioned_task_id = lastTaskId;

// Generate NEW session_id for next brain dump
context.session_id = generateUUID();

// Append to recent operations
...
**2. After Task Modifier completes:**
```javascript
if (modifierResult.status === "success") {
  // Update to modified task
  context.last_mentioned_task_id = modifierResult.matched_task.id;

  context.recent_operations.push({
    timestamp: new Date().toISOString(),
    operation_type: "update", // or "complete" or "delete"
    user_input: originalInput
  });
}
```

**3. After Query Agent completes:**
```javascript
// If query returned single specific task, optionally update
if (queryResult.count === 1) {
  context.last_mentioned_task_id = queryResult.matched_tasks[0].id;
}
...
**Session Concept:**
- A session = a single brain dump interaction (input → task creation)
- `session_id` = UUID generated by system for EACH brain dump request
- Used by Brain Dump Parser to tag tasks with `source_session`
- Deduplicator groups tasks by `source_session` to find duplicates

**Session Lifecycle:**
```javascript
// Before calling Orchestrator with "create" intent
if (routing.intent === "create") {
  // Generate new session ID
  context.session_id = generateUUID();
}

// Brain Dump Parser uses this session_id
const tasks = await brainDumpParser.parse(userInput, context);

// Each task now has: source_session = context.session_id

// After brain dump completes, session_id stays in context
// until next brain dump (when new UUID is generated)
```

### Context Persistence

**Stateful context across conversation:**
- Context persists across multiple user inputs in same conversation
- Reset `last_mentioned_task_id` after 5 minutes of inactivity
- Preserve `recent_operations` (last 5 only)
- `has_existing_tasks` should be updated after create/delete operations

**Context initialization (new conversation):**
```javascript
const context = {
  current_date: getCurrentDate(),      // e.g., "2025-11-16"
  current_time: getCurrentTime(),       // e.g., "14:30"
  current_day_of_week: getDayOfWeek(), // e.g., "Saturday"
  timezone: getUserTimezone(),          // e.g., "Europe/Kiev"
  last_mentioned_task_id: null,
  has_existing_tasks: await db.hasAnyTasks(),
  user_custom_categories: await db.getCustomCategories(), // e.g., ["mia", "startup"]
...
async function processUserInput(userInput, context) {
  ...
  const routing = await orchestrator.classify(userInput, context);

  // 2. Route to appropriate agent
  let result;
  switch (routing.routing_decision) {
    case "brain_dump_parser":
      // Generate new session_id for this brain dump
      context.session_id = generateUUID();
      result = await brainDumpParser.parse(userInput, context);
    ...
    case "task_modifier":
      ...
      result = await taskModifier.modify(userInput, existingTasks, context);

      if (result.status === "success") {
        // Apply changes to DB
        await db.updateTask(result.matched_task.id, result.changes);

        // Update context
        context.last_mentioned_task_id = result.matched_task.id;
      }
      break;

    case "query_agent":
      const tasks = await db.getAllTasks();
      result = await queryAgent.query(userInput, tasks, context);
      break;

    case "operation_splitter":
      const operations = await operationSplitter.split(userInput, context);

      // Execute each operation
      for (const op of operations.operations) {
        await processUserInput(op.text, context);
      }
      break;
  }

  // 3. Update recent operations
  context.recent_operations.push({
    timestamp: new Date().toISOString(),
    operation_type: routing.intent,
