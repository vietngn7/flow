...
A **session** represents a single brain dump interaction from user input to task creation.

**Session Properties:**
- `session_id`: UUID generated by system on each brain dump request
- **Lifetime**: One-time use (created → tasks generated → session ends)
- **Purpose**: Group tasks from the same brain dump for deduplication tracking

**Why sessions matter:**

When a user does multiple brain dumps, they often repeat tasks:

**Example:**
```
Session 1 (08:00 AM):
User: "купить молоко, позвонить маме"
Created:
  - task_001 (title: "Купить молоко", source_session: "session_abc123")
  - task_002 (title: "Позвонить маме", source_session: "session_abc123")

Session 2 (14:00 PM):
User: "купить молоко" (forgot they already added this!)
Created:
  - task_003 (title: "Купить молоко", source_session: "session_def456")

Deduplicator sees:
- task_001 and task_003 both about "молоко"
- Different source_session values → created at different times
- Same day, same category → likely duplicate!
→ Merge task_003 into task_001
```

**Who creates session_id?**
- System (application layer) generates a new UUID for each brain dump request
- Orchestrator passes `session_id` in context to Brain Dump Parser
- Brain Dump Parser includes it in each task's `source_session` field
- Deduplicator uses it to identify which tasks were created together

**Session vs Conversation:**
- Session = single brain dump (creates multiple tasks)
- Conversation = entire chat session (contains multiple brain dumps/sessions)
...
<invocation_strategy>
## When to Call the Deduplicator

**IMPORTANT:** The Deduplicator is NOT called on every brain dump. It runs periodically or on-demand to clean up accumulated duplicates.

### Strategy 1: Periodic (Recommended)

Run deduplicator automatically at regular intervals:

**Option A - After N brain dumps:**
```javascript
let brainDumpCounter = 0;

// After each brain dump
brainDumpCounter++;

if (brainDumpCounter >= 5) {
  // Get tasks from last 5 sessions
  const last5Sessions = getLastNSessionIds(5);
  const recentTasks = await db.getTasksBySessionIds(last5Sessions);

  // Run deduplicator
  const result = await deduplicator.deduplicate(recentTasks);
...
cron.schedule('0 23 * * *', async () => {
  ...
  const todayTasks = await db.getTasksByDate(getCurrentDate());

  if (todayTasks.length > 0) {
    const result = await deduplicator.deduplicate(todayTasks);
    await applyDeduplication(result);

    console.log(`Deduplicated ${result.duplicates_merged} tasks`);
  }
});
```

**Pros:** Low overhead, catches duplicates before they pile up
**Cons:** Duplicates exist for a few hours/sessions

---

### Strategy 2: On-Demand (User-Triggered)

Let users manually trigger deduplication:

**Command-based:**
```javascript
commands.register('/deduplicate', async () => {
  const allTasks = await db.getAllTasks({ completed: false });

  const result = await deduplicator.deduplicate(allTasks);
  await applyDeduplication(result);

  return `✓ Проверил задачи:\n` +
         `• Объединил: ${result.duplicates_merged} дубликатов\n` +
         `• Итого задач: ${result.metadata.total_output_tasks}`;
});
```

**Smart prompting:**
```javascript
// After brain dump, check for potential duplicates
const newTasks = brainDumpResult.tasks;
const existingTasks = await db.getAllTasks({ completed: false });

const potentialDuplicates = findSimilarTasks(newTasks, existingTasks);

if (potentialDuplicates.length > 3) {
  await askUser(
    "Заметил несколько похожих задач. Запустить дедупликацию?",
    ["Да, проверить", "Нет, оставить как есть"]
  );
}
```

**Pros:** User controls when to clean up
**Cons:** User might forget, duplicates accumulate

---

### Strategy 3: Threshold-Based (Smart Auto)

Automatically run when duplicate indicators exceed threshold:

```javascript
// Track duplicate indicators
class DuplicateTracker {
  async checkAfterBrainDump(newTasks) {
    const existingTasks = await db.getAllTasks({ completed: false });
    ...
    for (const newTask of newTasks) {
      for (const existingTask of existingTasks) {
        const similarity = calculateSimilarity(newTask.title, existingTask.title);
    ...
    // Threshold: run if 3+ potential duplicates detected
    if (duplicateScore >= 3) {
      console.log(`Detected ${duplicateScore} potential duplicates, running deduplicator...`);
      ...
      return {
        ranDeduplication: true,
        mergedCount: result.duplicates_merged
      };
    }

    return { ranDeduplication: false };
  }
}
```

**Pros:** Smart, only runs when needed
...
**For MVP:** Use Strategy 1 Option A (every 5 brain dumps)

**For production:** Combine strategies:
1. Auto-run every 5 brain dumps (catches most duplicates)
2. Daily batch at night (final cleanup)
3. User command `/deduplicate` available anytime

```javascript
// Combined approach
class DeduplicationManager {
  constructor() {
    this.brainDumpCounter = 0;
  }

  async afterBrainDump(tasks) {
    this.brainDumpCounter++;

    // Every 5 brain dumps
    if (this.brainDumpCounter >= 5) {
      await this.runDeduplication('periodic');
      this.brainDumpCounter = 0;
    }
  }

  async runDeduplication(trigger) {
    const tasks = await db.getAllTasks({ completed: false });

    if (tasks.length < 2) {
      return; // Need at least 2 tasks to deduplicate
    }

    const result = await deduplicator.deduplicate(tasks);
    ...
    if (result.duplicates_merged > 0) {
      ...
      logger.info('Deduplication completed', {
        ...
        conflicts: result.conflicts_resolved.length
      });

      // Notify user
      if (trigger === 'manual') {
        return `✓ Объединил ${result.duplicates_merged} дубликатов`;
      }
    }
  }
}
...
// Daily batch
cron.schedule('0 23 * * *', async () => {
  await deduplicationManager.runDeduplication('daily');
});

// Manual command
commands.register('/deduplicate', async () => {
  return await deduplicationManager.runDeduplication('manual');
});
```

---

### Integration with Brain Dump Flow

```javascript
async function handleBrainDump(userInput, context) {
  // 1. Generate new session_id
  context.session_id = generateUUID();

  // 2. Parse brain dump
  const result = await brainDumpParser.parse(userInput, context);
  ...
  // 4. Check if deduplication needed
  await deduplicationManager.afterBrainDump(result.tasks);

  // 5. Return to user
  return {
    tasks: result.tasks,
    deduplicationRan: deduplicationManager.brainDumpCounter === 0
  };
}
```
